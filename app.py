...
def get_people(location):
    ...
    for person in person_dirs:
        ...
        try:
            ...
            # 尋找 .txt 檔案 (強化修復: 使用 f.lower().endswith() 確保大小寫都能匹配)
            text_file = next((f for f in os.listdir(person_path) if f.lower().endswith('.txt')), None)
            if text_file:
                # 🚨 偵錯日誌：找到文件時在伺服器日誌中打印
                print(f"✅ Preview TXT found for {person}: {text_file}") 
